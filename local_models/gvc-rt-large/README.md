...
- `gvc-rt-large_tflite_online_dynamic_qp_270p.tar.gz`：当前正式在线模型包。10 张 TFLite 共用权重，运行时选择 QP 0、3、6、9；包含六张连续神经网络图、四张 entropy+rANS 图和七份量化尺度表。
...
当前部署固定输入尺寸为 `256x512`（270p 填充后的工作尺寸），支持运行时选择 QP 0、3、6、9，Tensor 布局为 NHWC、数据类型为 FP32。
