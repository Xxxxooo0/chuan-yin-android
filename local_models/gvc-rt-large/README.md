...
- `online_entropy/i_entropy_prior_merged.tflite`：I 帧 hyper/prior、四阶段掩码量化与反馈合并图。它保持为独立模型文件，不进入其他模型压缩包；部署时单独放到设备包的 `models/` 目录。Android 官方 NeuronDelegate 在线路径实测离散符号完全一致，10 次均值约 `37 ms`。
- `online_entropy/manifest.json`：独立合并模型的 SHA256、固定输入输出名称和 NHWC shape。
