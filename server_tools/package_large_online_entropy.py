#!/usr/bin/env python3
"""Append I/P CDF tables to a Large online package.

Merged I/P entropy encoder and decoder graphs are deployed as standalone
TFLite files. Obsolete split graphs are never copied into the package.
"""
...
def main() -> None:
    ...
    for name in OBSOLETE_I_MODELS + OBSOLETE_P_MODELS:
    ...
    manifest = {
        ...
        "i": {
            ...
            "decoder_cdf_mode": "all_packed_scales_android_encoder_compatible",
        },
        "p": {
            ...
            "z_per_channel_size": 32,
            "merged_models": {
                "encoder": "p_entropy_prior_merged_rans.tflite",
                "decoder": "p_entropy_decode_merged_rans.tflite",
                "packaging": "standalone_not_in_archive",
            },
        },
    }
    ...
    (output / "README.md").write_text(
        "# GVC-RT Large 在线熵编码包\n\n"
        "该压缩包在 Large 在线 TFLite 包基础上增加 I/P CDF 表。\n\n"
        "I/P 编码、解码的 merged+rANS TFLite 均作为独立文件部署，不进入本压缩包。"
        "部署时将四个独立文件放到包目录的 `models/`。旧的 I/P 分图不再保留。\n",
        encoding="utf-8",
    )
    write_checksums(output)
    print(f"wrote {output}")
    print("standalone_i_model=i_entropy_prior_merged sha256=" + merged_record["sha256"])
    print("standalone_p_models=p_entropy_prior_merged_rans,p_entropy_decode_merged_rans")
