...
def export_one(
    ...
    if name.startswith("decoder_synthesis"):
    ...
    if dla_path.exists():
        dla_path.unlink()
    with tempfile.TemporaryDirectory(prefix="gvc-rt-small-", dir="/tmp") as temporary_dir:
        compiled_dla_path = Path(temporary_dir) / (name + ".dla")
        compile_rc, compile_text = run(
            [ncc, str(tflite_path)] + ncc_flags + [
                ...
                "--show-memory-summary",
                "-d",
                str(compiled_dla_path),
            ],
            logs / "ncc_compile_dla.log",
            ncc_env,
        )
        if compile_rc == 0 and compiled_dla_path.is_file():
            shutil.copy2(str(compiled_dla_path), str(dla_path))
...
def publish_names(q_index: int, suffix: str) -> Dict[str, str]:
    return {
...
def package(output_dir: Path, records: List[Dict[str, Any]], checkpoint: Path, q_index: int) -> Path:
    ...
    manifest = {
        ...
        "reference_reset": {"period": 64, "phase": 1, "condition": "frame_index == 0 or frame_index % 64 == 1"},
        "models": published,
    }
    ...
    archive = output_dir / "gvc-rt-small_dla_codec_270p_qp9.tar.gz"
    with tarfile.open(str(archive), "w:gz") as handle:
        handle.add(str(delivery), arcname=package_name)
    return archive


def package_tflite(output_dir: Path, records: List[Dict[str, Any]], checkpoint: Path, q_index: int) -> Path:
    ...
    if delivery.exists():
        shutil.rmtree(str(delivery))
    models.mkdir(parents=True)
    names = publish_names(q_index, "tflite")
    published = []
    for record in records:
        if record.get("status") != "ok":
            raise RuntimeError("cannot package failed model: {}".format(record["name"]))
    ...
    (delivery / "manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    (delivery / "README.md").write_text(
        build_readme("dla", "MDLA 5.3 离线 DLA"),
        encoding="utf-8",
    )
    checksum_paths = sorted(list(models.glob("*.tflite")) + [delivery / "manifest.json", delivery / "README.md"])
    (delivery / "SHA256SUMS.txt").write_text(
        "".join("{}  {}\n".format(sha256(path), path.relative_to(delivery).as_posix()) for path in checksum_paths),
        encoding="ascii",
    )
    archive = output_dir / "gvc-rt-small_tflite_codec_270p_qp9.tar.gz"
    with tarfile.open(str(archive), "w:gz") as handle:
        handle.add(str(delivery), arcname=package_name)
    return archive
...
def main() -> None:
    ...
    if all(record.get("status") == "ok" for record in records):
        ...
        print("package={}".format(output_dir / "gvc-rt-small_dla_codec_270p_qp9"), flush=True)
