#!/usr/bin/env python3
"""Replace merged I entropy-decoder placeholder inputs with rANS custom ops."""
...
Z_CUSTOM_OP = "GVC_RT_RANS_DECODE_Z"
Y_CUSTOM_OP = "GVC_RT_RANS_DECODE_Y"
EXPECTED_INPUT_SHAPES = ([1, 4, 8, 128], *([1, 16, 32, 64] for _ in range(4)))
...
def add_custom_opcode(model_t, schema, name: str) -> int:
...
def append_decode_ops(
    ...
    if len(graph.inputs) != 5 or len(graph.outputs) != 10:
        raise RuntimeError("expected 5 inputs and 10 outputs, got {} and {}".format(len(graph.inputs), len(graph.outputs)))
    ...
    if input_shapes != tuple(EXPECTED_INPUT_SHAPES):
        raise RuntimeError("merged decoder input order mismatch: {}".format(input_shapes))
    if output_shapes != tuple(EXPECTED_OUTPUT_SHAPES):
        raise RuntimeError("merged decoder output order mismatch: {}".format(output_shapes))

    i_info = manifest["i"]
    gaussian_info = i_info["gaussian"]
    z_info = i_info["z"]
    gaussian_constants = [
        add_constant(model_t, graph, schema, "i_decode_gaussian_cdf", load_i32(package_root, gaussian_info["cdf"], gaussian_info["shape"])),
        add_constant(model_t, graph, schema, "i_decode_gaussian_lengths", load_i32(package_root, gaussian_info["cdf_lengths"])),
        add_constant(model_t, graph, schema, "i_decode_gaussian_offsets", load_i32(package_root, gaussian_info["offsets"])),
    ]
    z_constants = [
        add_constant(model_t, graph, schema, "i_decode_z_cdf", load_i32(package_root, z_info["cdf"], z_info["shape"])),
        add_constant(model_t, graph, schema, "i_decode_z_lengths", load_i32(package_root, z_info["cdf_lengths"])),
        add_constant(model_t, graph, schema, "i_decode_z_offsets", load_i32(package_root, z_info["offsets"])),
        add_constant(model_t, graph, schema, "i_decode_z_start_offset", np.asarray([i_info["z_start_offset"]], dtype="<i4")),
        add_constant(model_t, graph, schema, "i_decode_z_per_channel_size", np.asarray([i_info["z_per_channel_size"]], dtype="<i4")),
    ]
    payload_index = add_input(graph, schema, "i_rans_payload_buffer", [payload_capacity], int(schema.TensorType.UINT8))
    payload_size_index = add_input(graph, schema, "i_rans_payload_size", [1], int(schema.TensorType.INT32))
    graph.inputs = [payload_index, payload_size_index]

    z_opcode = add_custom_opcode(model_t, schema, Z_CUSTOM_OP)
    y_opcode = add_custom_opcode(model_t, schema, Y_CUSTOM_OP)
    z_op = custom_operator(
        schema,
        z_opcode,
        [payload_index, payload_size_index] + gaussian_constants + z_constants,
        [input_indices[0]],
    )
    ...
    for stage in range(4):
        ...
        scale_tensor = graph.outputs[stage + 5]
        q_consumers = consumers.get(q_tensor, [])
        if not q_consumers:
            raise RuntimeError("decoded y input stage {} has no consumer".format(stage))
        scale_producer = producer.get(scale_tensor)
        if scale_producer is None:
            raise RuntimeError("scale output stage {} has no producer".format(stage))
        insertion = min(q_consumers)
        if scale_producer >= insertion:
            raise RuntimeError(
                "cannot insert stage {} decode: scale producer {} is not before y consumer {}".format(
                    stage, scale_producer, insertion
                )
            )
    ...
    return {
        "name": "i_entropy_decode_merged_rans",
        "file": output.name,
        ...
        "input_names": ["i_rans_payload_buffer", "i_rans_payload_size"],
        "input_shapes": [[payload_capacity], [1]],
        "output_names": [
            "i_z_hat",
...
def main() -> None:
    ...
    record = append_decode_ops(
        args.merged_base_model.resolve(),
        args.output.resolve(),
        package_root,
        manifest,
        args.payload_capacity,
    )
    manifest_path = args.output.resolve().with_name("i_entropy_decode_merged_rans_manifest.json")
