...
本仓库的 Android 应用默认使用 **MTK 在线部署**：使用 TFLite 与官方 `NeuronDelegate` 在设备侧在线编译，通过外置模型包选择 GVC-RT Large 或 Small。另有标准 TFLite GPU 后端：显式 GPU 可强制探测兼容名单外设备；Small 从独立 `gpusmall` 六图包运行，Large 从独立 `gpularge` 十图包运行。连续神经网络图使用 GPU Delegate，GPU 不支持的 entropy builtin 可由普通 TFLite CPU kernel 执行，rANS 使用现有 native CPU 实现。选择方式、支持边界和真机验证见 [Small GPU 后端](docs/tflite-gpu-backend.md) 与 [Large GPU 十图路径](docs/tflite-gpu-large.md)。
