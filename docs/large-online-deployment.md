...
当前 Large 在线路径使用官方 `NeuronDelegate`。I 帧 entropy/prior 只使用独立模型
`i_entropy_prior_merged.tflite`；旧的 7 张 I 帧分图不再部署。P 帧 entropy/prior
暂时保留 4 张连续网络图，rANS 保持原生实现。
...
python -u server_tools/export_entropy_offline_nhwc.py \
  --source-root "$GVC_RT_SOURCE_ROOT" \
  --android-root "$PWD" \
  --output-dir outputs/p_entropy_online_source \
  --ncc-tflite neuropilot-sdk-premium-8.0.11-build20260211/neuron_sdk/host/bin/ncc-tflite \
  --candidates p_hyper_enc_continuous,p_hyper_prior_shared,p_prior_stage0_params,p_prior_stage1_continuous
...
python -u server_tools/package_large_online_entropy.py \
  ...
  --entropy-export-dir outputs/p_entropy_online_source \
...
打包目录不包含 merged 模型。该模型单独保存到本地模型分支的
...
& $adb push .\models\large\local_models\gvc-rt-large\online_entropy\i_entropy_prior_merged.tflite "$root/models/i_entropy_prior_merged.tflite"
```
